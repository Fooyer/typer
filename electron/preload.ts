...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  connections: {
    ...
    save: (profile: Omit<ConnectionProfile, "id"> & { id?: string }, password?: string): Promise<ConnectionProfile> =>
      ipcRenderer.invoke("connections:save", profile, password),
  ...
  atelier: {
    ...
    listNamespaces: (id: string): Promise<string[]> => ipcRenderer.invoke("atelier:listNamespaces", id),
    ...
    callRoute: (
      ...
      body?: string,
    ): Promise<RestCallResult> => ipcRenderer.invoke("atelier:callRoute", id, path, method, headers, body),
    ...
    getStudioMenus: (
      ...
      docName: string,
      selectedText?: string,
    ): Promise<StudioMenu[]> => ipcRenderer.invoke("atelier:getStudioMenus", id, namespace, menuType, docName, selectedText),
    invokeStudioUserAction: (
      ...
      ipcRenderer.invoke("atelier:invokeStudioUserAction", id, namespace, type, actionId, docName, selectedText),
    invokeStudioAfterUserAction: (
      ...
      ipcRenderer.invoke("atelier:invokeStudioAfterUserAction", id, namespace, type, actionId, docName, answer, msg),
  },
  studio: {
    openCspAction: (url: string): Promise<"1" | "2"> => ipcRenderer.invoke("studio:openCspAction", url),
  },
  files: {
